...
'''
计算一个panel上单位源source强度或者单位偶极子doublet强度对某个点的速度贡献
'''

def Src_disturb_velocity(r_p:Vector, panel:Panel, alpha=10):   # panel上单位源强度对r_p处速度之贡献
    ...
    r = r_p - r_cp
    r_local = r.transformation(R)                             # 首先先变换到panel在z=0上的好情况来
    r = r_local
    '''
    我们对于几种情况分别计算速度之贡献
    1. 远场，判断标准为 ||r||即r_p距离panel中心点cp的距离超过alpha倍的panel最大对角线长
    2. 中心点 直接使用结论:σ/2就可以了
    3. 在平面上而非中心点 区别到底是在panel上还是在panel同个平面的别的位置
    4. 其他情况，即并不离开panel很远，但也并不与panel共面:使用Hess-Smith积分计算
    '''
    ...
    disturb_velocity = disturb_velocity_local.transformation(R.T)   # 记得要转回来啊
    
    return disturb_velocity

def Dblt_disturb_velocity(r_p:Vector, panel:Panel, alpha=10): # panel上单位偶极子强度对r_p处速度之贡献
    n = panel.num_vertices
    r_vertex = panel.r_vertex_local
    r_cp = panel.r_cp
    R = panel.R 
